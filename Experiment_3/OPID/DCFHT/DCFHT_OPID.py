...
class OCFHT_drift_new():
    ...
    def OCFHT_drift_OVFM(self, X, Y):
        ...
        return self.classifier, self.err_count, self.correct_cnt, self.correct_cnt_E, runtime, acc_all
    ...
    def predict(self, x_t, y_t, idx_t, x_t_):
        ...
        if y_t == pred_t:
            ...
            if self.all_instances > self.num_C + np.round(0.6*self.num_E):
                self.correct_cnt_E += 1
        else:
            self.err_count += 1
            pred_result = 1
        ...
        if self.instances_seen == 1: 
            ...
            if self._use_drift_detector and not self.is_background_learner:
                if self._use_background_learner:
                    self.warning_detection.add_element(pred_result)
                ...
                self.drift_detection.add_element(pred_result)
